...
export type AniListMedia = {
  ...
  format?: 'TV' | 'TV_SHORT' | 'OVA' | 'ONA' | 'SPECIAL' | 'MOVIE' | string | null;
...
async function fetchAniListData<T>(query: string, variables: Record<string, any> = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`AniList API responded with status ${res.status}: ${errorText}`);
  }
...
/** One page of Popular */
export async function getPopularAnime(page = 1, perPage = 20, isAdult = false, filters: FormatFilters = {}) {
  ...
  const variables = hasFilters 
    ? { page, perPage, isAdult, format: filters.format ?? null, formatIn: filters.formatIn ?? null }
    : { page, perPage };
...
export async function getTrendingAnime(page = 1, perPage = 20, isAdult = false, filters: FormatFilters = {}) {
  // Build the query based on whether filters are provided
  const hasFilters = filters.format || filters.formatIn;
...
export async function getTopRatedAnime(page = 1, perPage = 20, isAdult = false, filters: FormatFilters = {}) {
  // Build the query based on whether filters are provided
  const hasFilters = filters.format || filters.formatIn;
...
export async function getUserCustomLists(userName: string, type: 'ANIME' | 'MANGA' = 'ANIME') {
  const query = `
    ${MEDIA_FIELDS}
  ...
  `;
  const data = await fetchAniListData<MediaListCollectionResponse>(query, { userName, type });
