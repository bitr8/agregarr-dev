  ...
  constructor(settings: OverseerrSettings) {
    ...
    this.axios.interceptors.response.use(
      ...
      (error) => {
        ...
        if (error.config?.url) {
          try {
            const url = new URL(error.config.url, error.config.baseURL || 'http://localhost');
            // Remove sensitive query params
            const sensitiveParams = ['api_key', 'apikey', 'token', 'key', 'secret', 'password', 'X-Plex-Token'];
        ...
        const safeHeaderKeys = ['content-type', 'accept', 'user-agent', 'content-length'];
        const safeHeaders: Record<string, unknown> = {};
        if (error.config?.headers) {
          // Flatten any nested header structures (Axios can nest headers)
          const flatHeaders = typeof error.config.headers.toJSON === 'function'
        ...
        if (error.response?.data) {
          try {
            ...
            if (typeof error.response.data === 'string') {
              ...
              dataStr = error.response.data.length > 2000
              ...
              dataStr = limited.length > 2000 ? limited.substring(0, 2000) : limited;
            }
