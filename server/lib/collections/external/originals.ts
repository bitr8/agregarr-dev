...
export class OriginalsCollectionSync extends BaseCollectionSync {
  ...
  protected async generateAutoPoster(
    ...
    try {
      ...
      logger.info(
        `Auto-generating poster for Originals collection: ${collectionName}`,
        {
          label: 'Originals Collections',
          configId: config.id,
          platform: platformName,
      ...
      if (items && items.length > 0) {
        const originalsItems = items as OriginalsCollectionItem[];
        posterItems = originalsItems.slice(0, 100).map((item) => ({
          title: item.title,
          type: item.type,
          tmdbId: item.tmdbId,
          year: item.year,
      ...
      const collectionIdentifier = config.id;

      // Generate the poster with platform-specific branding
      const posterConfig = {
        collectionName,
        collectionType: platformName, // Use platform name for proper theming
      ...
      // Get the uploaded poster URL from Plex for workflow completion
      ...
      logger.info(
        `Successfully generated and applied poster for Originals collection: ${collectionName}`,
        {
          label: 'Originals Collections',
          platform: platformName,
        }
      ...
      logger.error(
        `Failed to generate autoPoster for Originals collection: ${collectionName}`,
        {
          label: 'Originals Collections',
