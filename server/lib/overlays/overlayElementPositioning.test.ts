...
const withPosterizarr = async (fn: () => Promise<void>) => {
...
describe('overlay element positioning', () => {
  it("keeps an ownership marker inside Posterizarr's first-64-KiB scan", () =>
    withPosterizarr(async () => {
      const width = 1000;
      const height = 1500;
      ...
      const output = await overlayTemplateRenderer.compositeOverlays(
        source,
        []
      );
      ...
      expect(getRecognizedPosterOwnershipMarker(source)).toBe(
        posterizarrMarker
      );
  ...
  it('honours the configured JPEG quality and keeps full chroma detail', () =>
    withPosterizarr(async () => {
  ...
  it('encodes WebP with no ownership marker when Posterizarr is off', async () => {
    const poster = await sharp({
      create: { width: 200, height: 300, channels: 3, background: '#123456' },
    })
      .png()
      .toBuffer();
    const output = await overlayTemplateRenderer.compositeOverlays(poster, [
      await renderOne(
        tile(undefined, { x: 10, y: 10, width: 50, height: 50 }),
        200,
        300
      ),
    ]);
    expect(output.subarray(0, 4).toString()).toBe('RIFF');
    expect(output.toString('latin1')).not.toContain(AGREGARR_OVERLAY_MARKER);
  });
